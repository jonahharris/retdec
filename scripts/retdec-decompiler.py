        ...
        if self.mode in ['bin', 'raw']:
            ...
            if self.arch in ['arm', 'thumb']:
                ...
                utils.print_error('Unsupported target architecture \'%s\'. Supported architectures: '
                                  'Intel x86, x86-64, ARM, ARM + Thumb, MIPS, PIC32, PowerPC.' % self.arch)
                return 1
            ...
            if fileclass not in ['16', '32', '64']:
                ...
                self._cleanup()
                utils.print_error(
                    'Unsupported target format \'%s%s\'. Supported formats: ELF32, ELF64, PE32, PE64, Intel HEX 32, Mach-O 32, Mach-O 64.' % (
